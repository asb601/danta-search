...
function groupByTime(convs: ConversationSummary[]) {
...
const drawerVariants = {
  ...
  show: { x: 0, opacity: 1, transition: { duration: 0.22, ease: "easeOut" as const } },
  exit:  { x: -16, opacity: 0, transition: { duration: 0.16, ease: "easeIn"  as const } },
};
...
export function ConversationSidebar({
  ...
  isOpen, onToggle, searchQuery, onSearchChange,
}: {
    ...
    <>
      ...
      <div className="px-3 pt-3.5 pb-2.5 border-b border-[#e5e5e5] flex items-center justify-between gap-2 shrink-0">
        ...
        <div className="flex items-center gap-0.5 shrink-0">
          ...
          {/* Close button — visible on mobile only (desktop sidebar is always open) */}
          <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.92 }}
            onClick={onToggle}
            className="sm:hidden p-1.5 rounded-md text-[#a3a3a3] hover:text-[#0a0a0a] hover:bg-[#ebebeb] transition-colors"
            title="Close"
          >
        ...
        <AnimatePresence mode="wait">
          {conversations.length === 0 ? (
            <motion.div key="empty" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
              transition={{ duration: 0.3 }} className="px-4 py-10 text-center"
            >
              ...
              {grouped.map(({ label, items }) => (
                <div key={label}>
                  <div className="px-3 pt-3 pb-1">
                    <span className="text-[10px] font-semibold text-[#c4c4c4] uppercase tracking-[0.08em]">{label}</span>
                  </div>
                  {items.map((conv) => {
                    const i = idx++;
                      ...
                      >
                        ...
                        {editingId === conv.id ? (
                          ...
                          <div className="flex items-center gap-1.5" onClick={(e) => e.stopPropagation()}>
                            ...
                            <button onClick={() => { onDelete(conv.id); setConfirmDeleteId(null); }}
                              className="px-2 py-0.5 text-[11px] font-semibold rounded-md bg-[#dc2626] text-white hover:opacity-90">Yes</button>
                            <button onClick={() => setConfirmDeleteId(null)}
                              className="px-2 py-0.5 text-[11px] font-medium rounded-md border border-[#e5e5e5] text-[#737373] hover:text-[#0a0a0a]">No</button>
                          </div>
                        ) : (
                          <>
                            <p className="text-[12px] font-medium text-[#0a0a0a] truncate pr-10 leading-snug">{conv.title}</p>
                            <div className="flex items-center gap-1.5 mt-0.5">
                              <Clock className="w-2.5 h-2.5 text-[#c4c4c4] shrink-0" />
                              <p className="text-[10.5px] text-[#a3a3a3]">{relativeTime(conv.updated_at)}</p>
                              {conv.message_count > 0 && (
                                <>
                                  <span className="text-[#d4d4d4]">·</span>
                                  <p className="text-[10.5px] text-[#a3a3a3]">{conv.message_count} msg{conv.message_count !== 1 ? "s" : ""}</p>
                                </>
                              )}
                            </div>
                            {/* Hover actions */}
                            <div className="absolute right-1.5 top-1/2 -translate-y-1/2 hidden group-hover:flex items-center gap-0.5 bg-white border border-[#e5e5e5] rounded-md px-0.5 shadow-sm">
                              <button onClick={(e) => { e.stopPropagation(); startRename(conv); }}
                                className="p-1 rounded text-[#a3a3a3] hover:text-[#0a0a0a] transition-colors" title="Rename">
                                <Pencil className="w-3 h-3" />
                              </button>
                              <button onClick={(e) => { e.stopPropagation(); setConfirmDeleteId(conv.id); }}
                                className="p-1 rounded text-[#a3a3a3] hover:text-[#dc2626] transition-colors" title="Delete">
  ...
  return (
    <>
      ...
      <div className="hidden sm:flex flex-col w-[240px] shrink-0 border-r border-[#e5e5e5] bg-[#f9f9f9] h-full overflow-hidden">
        {body}
      </div>

      {/* ── Mobile: overlay drawer (only when isOpen) ──────────────── */}
      <AnimatePresence>
        {isOpen && (
          <>
            <motion.div
            ...
            <motion.div
              ...
              animate="show"
              exit="exit"
              className="sm:hidden fixed inset-y-0 left-0 z-40 w-[260px] flex flex-col bg-[#f9f9f9] border-r border-[#e5e5e5] shadow-[4px_0_24px_rgba(0,0,0,0.08)] overflow-hidden"
            >
              {body}
            </motion.div>
          </>
